...
export const TechDocsModal: React.FC<TechDocsModalProps> = ({
  isOpen = true,
  onClose,
  isSubPage = false,
}) => {
    ...
    <>
        ...
        {activeSection === 'update' && (
          <div className="animate-fade-in">
            ...
            <div style={{ background: 'var(--bg-input)', padding: '12px', borderRadius: '8px', marginBottom: '10px' }}>
              ...
              <div style={{ fontSize: '0.78rem' }}>
                开发者只需在 Git 仓库打上版本标签（如 <code>v1.1.0</code>）推送到远程，GitHub Actions 自动启动 Ubuntu Runner 执行 Node 22 与 Gradle 纯命令行构建，编译生成标准命名的 <code>IronTrack-vX.X.X.apk</code>，并通过 <code>softprops/action-gh-release</code> 自动创建 Release 挂载安装包资产。
              </div>
            </div>
      ...
      <div style={{ marginTop: '14px', paddingTop: '10px', borderTop: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '0.74rem', color: 'var(--text-muted)' }}>
          详细文档请查看项目根目录 <code>TECH_ARCHITECTURE.md</code>
        </span>
        {onClose && !isSubPage && (
